...
设计要点:
- 连接池/SQL: psycopg3 优先, psycopg2 回退
- add/search/get_all/delete 契约对齐 MemoryStore ABC
- history 表记录 memory_id/old_memory/new_memory/event/created_at/is_deleted
- 组合 BM25: PG 无内置 BM25, 用 SQLiteBM25Index 辅助
- 权限层: state 状态机 + memory_access_logs 审计表
"""
...
def _with_sslmode(connection_string: str, sslmode: str) -> str:
    """在 URI 或 keyword conninfo 中添加/替换 sslmode 参数。
...
class PGVectorStore(MemoryStore):
    ...
    def _create_tables(self) -> None:
        ...
        with self._get_cursor(commit=True) as cur:
            ...
            # history 表 (变更历史: memory_id/old_memory/new_memory/event/created_at/is_deleted)
            cur.execute(
            ...
            # P2 Task 7: memory_access_logs 审计表
            cur.execute(
    ...
    def search(
        ...
        *,
        ...
        filters: 字段过滤字典, 暂未实现 (接受参数避免 TypeError, 后续补齐)。
        """
        self._ensure_collection()
        with self._get_cursor() as cur:
            cur.execute(
    ...
    def delete(self, memory_id: str) -> None:
        """软删除 (is_deleted=1 + state='deleted' + deleted_at + history)。
    ...
    # ------------------------------------------------------------------
    # 关系查询 (跨 agent 共享, 对齐 ORMMemoryStore)
    # ------------------------------------------------------------------
    ...
    def __del__(self) -> None:
        """析构时静默关闭 (对齐 ORMMemoryStore.__del__)。"""
        with contextlib.suppress(Exception):
            self.close()
