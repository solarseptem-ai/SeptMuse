...
class DatabaseConfig(BaseModel):
    ...
    零配置: db_path=None → 默认 ~/.septmuse/septmuse.db。
    内存库: db_path=":memory:"。
    PostgreSQL: db_path 设为 PG DSN (postgresql://user:pass@host:port/db)。
    """

    db_path: str | Path | None = Field(
        default=None,
        description="SQLite 路径; None → 默认 ~/.septmuse/septmuse.db; ':memory:' → 内存库; PostgreSQL DSN → PG 模式",
    )
