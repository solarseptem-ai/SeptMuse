...
| 模块 | 自研理由 | 复杂度 | 验证方式 |
|------|---------|:-----:|---------|
| **因果链记忆** | 14 家均无因果边 + 反事实查询 | 高 | 因果测试集召回率 + 反事实准确率 |
| **Ebbinghaus 遗忘曲线** | 14 家均无强度衰减 + 主动复述 | 中 | 衰减曲线拟合 + 复述后强度回升 |
| **元认知 L1 覆盖自描述** | ReMe 仅 L0 路由, 无覆盖自述 | 中 | 覆盖报告准确性 + 策略自调效果 |
| **元认知 L2 策略自调** | 全新, 基于覆盖报告驱动 | 中 | 澄清提问触发合理性 |
| **源同步器** | 多形态共存一致性, 14 家均无统一方案 | 中 | 形态间漂移检测 + 补偿 |
| **统一编排 API** | 六层记忆的统一入口 + 路由 | 中 | 端到端记忆回路集成测试 |
| **置信度 + 溯源** | Cass 仅规则退化, 事实/推断未区分 | 低 | 置信度加权排序 A/B |
| **跨 agent 权限治理** | Agno 仅共享无权限, 需 RBAC | 中 | 权限矩阵测试 |
